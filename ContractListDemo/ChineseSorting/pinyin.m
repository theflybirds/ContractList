...
#define HANZI_START1 19968
#define HANZI_COUNT1 20902

#import "pinyin.h"

static char firstLetterArray2[HANZI_COUNT1] =
...
{
	int index = hanzi - HANZI_START1;
	if (index >= 0 && index <= HANZI_COUNT1)
	{
		return firstLetterArray2[index];
	}
	else
	{
		return '#';
...
+ (NSString *)firstLetterUsingSeperate:(NSString *)seperate chineseString:(NSString *)chineseString
{
    ...
    [chineseString enumerateCharaterUsingBlock:^(unsigned short letter, BOOL *stop) {
     ...
     int index = letter - HANZI_START1;
     
     if (index >= 0 && index <= HANZI_COUNT1){
     
        if ([firstLetters length]){
            firstLetters = [firstLetters stringByAppendingFormat:@" %c", firstLetterArray2[index]];
        }else {
            firstLetters = [firstLetters stringByAppendingFormat:@"%c", firstLetterArray2[index]];
        }
