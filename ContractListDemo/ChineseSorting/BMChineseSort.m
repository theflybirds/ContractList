...
+(NSMutableArray*)returnItem:(NSMutableArray*)stringArr name:(Person *)name{
    
    
    for (int i = 0;i<stringArr.count;i++) {
        
        Person *object = stringArr[i];
        
        NSString *xing = [object.name substringToIndex:1];
        //不同
        if([[name.name substringToIndex:1] isEqualToString:xing])
        {
...
+ (NSString *)getFirstLetter2:(NSString *)chinese{
    ...
    for(int j=0;j<chinese.length;j++){
        NSString *singlePinyinLetter = [[NSString stringWithFormat:@"%c",
                                         pinyinFirstLetter1([chinese characterAtIndex:j])]uppercaseString];
...
"xy";
char pinyinFirstLetter1(unsigned short hanzi)
{
