...
+(NSMutableArray*)mIndexArray:(NSArray*)stringArr
{
    ...
    {
        if ([((ChineseString*)object).pinYin isEqual:@""]) {
            ((ChineseString*)object).pinYin = @"YIMING";
            ((ChineseString*)object).string = @"佚名";
        }
        NSString *pinyin = [((ChineseString*)object).pinYin substringToIndex:1];
        //不同
        if(![tempString isEqualToString:pinyin])
...
{
    NSMutableArray *tempArray = [self mReturnSortChineseArrar:stringArr];
    NSMutableArray *LetterResult=[NSMutableArray array];
    NSMutableArray *item = [NSMutableArray array];
    NSString *tempString ;
    //拼音分组
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((ChineseString*)object).pinYin substringToIndex:1];
        NSString *xing = [((ChineseString*)object).string substringToIndex:1];
...
+(NSMutableArray*)returnItem:(NSMutableArray*)stringArr name:(NSString *)name{
    
    
    for (int i = 0;i<stringArr.count;i++) {
        
        NSString *object = stringArr[i];
        
        NSString *xing = [object substringToIndex:1];
        //不同
        if([[name substringToIndex:1] isEqualToString:xing])
        {
            [stringArr insertObject:name atIndex:i];
            return stringArr;
        }
    }
    
    [stringArr addObject:name];
    return stringArr;
}
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        chineseString.string=[stringArr objectAtIndex:i];
            ...
            if(![chineseString.string isEqualToString:@""]){
                ...
                for(int j=0;j<chineseString.string.length;j++){
                    NSString *singlePinyinLetter=[[NSString stringWithFormat:@"%c",
                                                   ...
                                                   pinyinFirstLetter2([chineseString.string characterAtIndex:j])]uppercaseString];
...
//
///////////////////
+(NSMutableArray*)mReturnSortChineseArrar:(NSArray*)stringArr
{
    //获取字符串中文字的拼音首字母并与字符串共同存放
    NSMutableArray *chineseStringsArray=[NSMutableArray array];
    for(int i=0;i<[stringArr count];i++)
